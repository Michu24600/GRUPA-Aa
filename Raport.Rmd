...
**🎯 Cel Analizy** Głównym celem niniejszego projektu jest identyfikacja oraz analiza kluczowych determinant kształtujących ceny nieruchomości w Polsce w roku 2024. Projekt skupia się na zbadaniu zależności między cechami fizycznymi i lokalizacyjnymi mieszkań a ich wyceną rynkową.

Cały proces analityczny obejmuje kompleksowe podejście do danych: od ich czyszczenia (Data Cleaning) i inżynierii cech (Feature Engineering), poprzez eksploracyjną analizę danych (EDA) i wizualizację przestrzenną, aż po modelowanie statystyczne.

## 1.1 Pytania Badawcze

W toku analizy staramy się znaleźć odpowiedzi na następujące pytania:
...
## 1.3 📖 Słownik Zmiennych (Data Dictionary)
...
```{r Wykres obrazujący naruszenia, echo=FALSE, message=FALSE, warning=FALSE, fig.height=8,fig.width=16}
